...
`cProfile` 對 production 設定（`my_optimizer.py` 目前的參數）跑真實資料
剖析，發現 legalize 階段的效能熱點高度集中：`_FreeRectPool.occupy()`
（`legalize_lff` 內部維護 MAXRECTS 自由矩形池的核心方法，每放一個 block
就呼叫一次）裡剔除「被別的矩形完全包含」與「重複」矩形的邏輯，是純
Python 雙層迴圈——對每個新產生的 leftover 矩形，逐一呼叫 `_rect_contains`
比對其餘所有矩形。剖析單一 2-sample 片段就測到 `_rect_contains` 被呼叫
超過 200 萬次，`occupy()` 佔掉將近九成的 legalize 總時間，且對 block
數多、outline 需要多次重試的樣本（例如某 k=116 的真實樣本，legalize
單獨要 3.8s）尤其明顯。

**改法**：新增 `_prune_contained_rects()`，把同一套判定邏輯改寫成
numpy 向量化版本：(1) 重複矩形用 dict 找「第一次出現」的位置，O(n) 雜湊
取代 O(n²) 的逐一 `r2 == r` 比對；(2) 「被其他矩形嚴格包含」的判定，用
一次 n×n 的向量化陣列比較（`x0/y0/x1/y1` 各自 broadcast 成 n×n 矩陣）
取代 Python 迴圈裡逐一呼叫 `_rect_contains`。跟原本的判定式在數學上
完全等價，只是用陣列運算取代大量小函式呼叫的 Python 開銷——沒有改動
任何演算法決策、候選搜尋順序、或任何會影響輸出的邏輯。

**驗證**：
- 2000 組隨機合成案例（含刻意注入的重複矩形、巢狀包含等對抗案例）：
  新舊兩版函式逐一比對輸出，**0 個不一致**（含順序）。
- 完整 v5.35 單元/fuzz 測試套件重跑一次：結果與改動前完全相同（20/20
  逐位元反向相容、40/40 group 保證成立、20/20 fallback 正確、20/20
  fixed_mask 正確、50/50 fuzz 通過）。
- 4 個真實資料樣本（tid=0/20/50/80）：monkeypatch 切換新舊
  `_prune_contained_rects`，同一組隨機種子分別跑一次完整
  `run_one_sample`，**legalized 的 (x,y,w,h) 逐位元完全相同**——這是
  這次改動最直接的正確性證據，不是靠統計上「應該差不多」推論的。

**效能結果**（同一組 production 設定，100 樣本）：
- avg legalize time：0.997s → 0.575s（**-42%**）
- avg total runtime：1.54s → 1.10s（**-29%**）
- 官方 evaluate（100 樣本、1 次，因為輸出已證明逐位元不變、沒有品質
  取捨要篩選，只需要確認真實環境下 runtime 確實下降即可，不需要像
  一般實驗那樣跑 2-3 次看變異度）：avg runtime 0.96s，換算真實 median
  runtime 分數 **1.0995**，比 v4 baseline（1.128，v5.17 確認時的數字）
  再進步約 **-2.5%**——純粹來自 RuntimeFactor 項下降，area_gap／
  hpwl_gap／V_relative 完全不變（輸出本來就逐位元相同）。

**決定**：**採用**。這是這個 session 少見的「零風險」改動——不像其他
版本要在品質/代價之間做取捨判斷，這裡的正確性是直接證明（逐位元比對）
而非統計推論，效能提升也直接反映在官方 evaluate 的真實分數上。
