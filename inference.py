...
def generate_floorplan(
    ...
    use_self_cond=False,
    # v5.30: 純推論端實驗，只用於 ddim 分支——見 diffusion.py:
...
def legalize_result(
    ...
    # v5.35（實驗用，預設關閉）：見 utils.py:
    # legalize_lff docstring 的 use_atomic_group_placement 說明。
    ...
    # v5.22（實驗用）：見 utils.py: compact_swap docstring
    use_swap=False,
    swap_sweeps=3,
    # v5.23（實驗用）：見 utils.py: compact_anneal docstring
    use_anneal=False,
    anneal_iters=500,
    anneal_seed=0,
    # v5.24（實驗用）：見 utils.py: compact_seqpair docstring
    use_seqpair=False,
    seqpair_iters=500,
    seqpair_seed=0,
    ...
    # v5.27（實驗用）：見 utils.py: compact_seqpair_grouped docstring
    ...
    def _run_once(mode):
        xa, ya, wa, ha = legalize_lff(
            ...
            use_atomic_group_placement=use_atomic_group_placement,
            ...
            use_seqpair_grouped=use_seqpair_grouped,
            use_seqpair_grouped_end=use_seqpair_grouped_end,
            seqpair_grouped_iters=seqpair_grouped_iters,
            seqpair_grouped_seed=seqpair_grouped_seed,
            seqpair_grouped_relax_boundary=seqpair_grouped_relax_boundary,
...
def run_one_sample(sample_idx, official, model, config, device,
                   ...
                   use_seqpair_grouped=False, use_seqpair_grouped_end=False,
                   seqpair_grouped_iters=500, seqpair_grouped_seed=0,
                   seqpair_grouped_relax_boundary=False,
